//Run this with 'root -l -b' command!
...
for(unsigned int i=0; i<User_ID.size(); i++){
  ...
  for(int i=0; i<21; i++){
    start = 0.01+0.001*i;
    TF1 *gr1_fit3 = new TF1("gr1_fit3","pol1",start,0.04);
    gr1->Fit(gr1_fit3,"RQ");

    double par[2];
    gr1_fit3->GetParameters(par);
    cout << "low pt fitting slope in IB region : " << par[1] << endl;
  }

//  TF1 *gr1_fit1 = new TF1("gr1_fit1","pol1",0,0.0075);
//  TF1 *gr1_fit2 = new TF1("gr1_fit2","pol1",0.0075,0.012);
//  TF1 *gr1_fit3 = new TF1("gr1_fit3","pol1",0.012,0.04);
//
