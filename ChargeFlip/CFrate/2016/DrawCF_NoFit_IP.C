...
for(unsigned int i=0; i<User_ID.size(); i++){
  ...
  TH1D* h1 = (TH1D*)f1->Get(User_ID.at(i)+"/CFrate_IP/Denom");
  TH1D* h2 = (TH1D*)f1->Get(User_ID.at(i)+"/CFrate_IP/Num");
  
  // Let's use vector instead of array, to remove zero bins and nan bins //
  
  vector<double> X_1, EX_1;
  for (int i=0; i<200; i++) {
    X_1.push_back(-0.1+(2*i+1)*(0.2/400.)); EX_1.push_back(0.2/400.); 
  }
  ...
  //TCanvas* c1 = new TCanvas("c1","ChargeFlip_IP ("+User_ID.at(i)+")",200,200,900,800);
  TCanvas* c1 = new TCanvas("c1","ChargeFlip_IP",200,200,900,800);
  ...
  gr1->SetLineColor(15);
  gr1->SetTitle("ChargeFlip_IP");
  gr1->SetMinimum(0.);
  gr1->Draw("ZAP"); //Z : do not draw small horizontal/vertical lines the end of the error bars
  gr1->GetXaxis()->SetRangeUser(-0.1,0.1);
  gr1->GetXaxis()->SetTitle("#scale[0.8]{IP [cm]}");
