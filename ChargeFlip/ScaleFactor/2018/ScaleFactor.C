...
void ScaleFactor(TString id, TString EtaRegion, TString fit, int Syst = 0, TString SaveAs = "n"){ // BB, BE, EE
  ...
  TString filename = "/data6/Users/jihkim/SKFlatOutput/Run2Legacy_v4/ChargeFlip/2018/ScaleFactor__/DATA/ChargeFlip_EGamma.root";
  ...
  if(fit=="BW_pol1"){
    ...
    if(EtaRegion=="BB") fitFcn->SetParameters(312,7.,-90,-20,0.9,0.,0.); //optimized - chi2/ndf : 1079.22/23, SF : 135.4%
    else if(EtaRegion=="EE") fitFcn->SetParameters(3012.7,2.,-90,-9.,4.,0.,0.); //Don't use this - chi2/ndf : 5138.72/23, SF : ..., <--- negative bkg 1020
    ...
    if(EtaRegion=="BB") fitFcn->SetParameters(312,7.,-90,2,-0.9); //optimized - chi2/ndf : 1394.63/25, SF : 136.3% <--- 35% bkg.
    else if(EtaRegion=="EE") fitFcn->SetParameters(312.7,2.,-90,4.,-2.); //Don't use this	- chi2/ndf : 180587/25, SF : 152%
  ...
  if(fit=="gaus_pol1"){
    ...
    if(EtaRegion=="BB") fitFcn->SetParameters(480,90,3,-40,0.9,0.,0.); //optimized - chi2/ndf : 1163.56/23, SF : 85.6% XXX <--- 60% bkg.
    else if(EtaRegion=="EE") fitFcn->SetParameters(480,90,3,-40,0.9,0.,0.); //optimized - chi2/ndf : 3855.66/23, SF : 109.372% XXX <--- 25% bkg.
    else if(EtaRegion=="BE") fitFcn->SetParameters(480,90,3,-40,0.9,0.,0.); //optimized - chi2/ndf : 3887.03/23, SF : 89.3% SF
    ...
    else if(EtaRegion=="EE") fitFcn->SetParameters(480,90,3,4,-0.9); //FIXME chi2/ndf : /25, SF : %
