...
TauIDSFTool::TauIDSFTool(const std::string& year, const std::string& id, const std::string& wp, const bool dm, const bool embedding): tauID(id), tauWP(wp){

  bool verbose = false;
  std::string datapath                = getenv("DATA_DIR");
  datapath += "/Tau";
  ...
  if(std::find(antiJetIDs.begin(),antiJetIDs.end(),tauID)!=antiJetIDs.end()){
    if(dm){
      TString filename;
      if (embedding) {
          if (tauID.find("oldDM") != std::string::npos)
          {
             std::cerr << "Scale factors for embedded samples are not provided for the MVA IDs." << std::endl;
             assert(0);
          }
          filename = Form("%s/TauID_SF_dm_%s_%s_EMB.root",datapath.data(),tauID.data(),year.data());
      }
      else {
          filename = Form("%s/TauID_SF_dm_%s_%s.root",datapath.data(),tauID.data(),year.data());
      }
      TFile* file = ensureTFile(filename,verbose);
      hist = extractTH1(file,tauWP);
      ...
      if (embedding) {
          ...
          filename = Form("%s/TauID_SF_pt_%s_%s_EMB.root",datapath.data(),tauID.data(),year.data());
      }
      else {
          filename = Form("%s/TauID_SF_pt_%s_%s.root",datapath.data(),tauID.data(),year.data());
      }
      TFile* file = ensureTFile(filename,verbose);
      func[""]     = extractTF1(file,Form("%s_cent",tauWP.data()));
      func["Up"]   = extractTF1(file,Form("%s_up",  tauWP.data()));
      func["Down"] = extractTF1(file,Form("%s_down",tauWP.data()));
      file->Close();
      delete file;
      isVsPT = true;
    }
  }else if(std::find(antiEleIDs.begin(),antiEleIDs.end(),tauID)!=antiEleIDs.end()){
      if (embedding){
          std::cerr << "SF for ID " << tauID << " not available for the embedded samples!" << std::endl;
          assert(0);
      }
      TString filename = Form("%s/TauID_SF_eta_%s_%s.root",datapath.data(),tauID.data(),year.data());
      TFile* file = ensureTFile(filename,verbose);
      hist = extractTH1(file,tauWP);
      hist->SetDirectory(nullptr);
      file->Close();
      ...
      genmatches = {1,3};
      isVsEta    = true;
  }else if(std::find(antiMuIDs.begin(),antiMuIDs.end(),tauID)!=antiMuIDs.end()){
      if (embedding){
          std::cerr << "SF for ID " << tauID << " not available for the embedded samples!" << std::endl;
          assert(0);
      }
      ...
      std::cerr << "Did not recognize tau ID '" << tauID << "'!" << std::endl;
      assert(0);
  }
}
