#include "WRTau_FakeRate.h"

void WRTau_FakeRate::initializeAnalyzer(){

  //WRTau_Core::initializeAnalyzer();
...
void WRTau_FakeRate::executeEvent(){
  ...
  vector<WRTau_Core::Channel> channel;
  ...
  ELIDs.push_back(make_pair("passHEEPID", "passVetoID"));  // FIXME : fix to WRTau Electron IDs
  paramnames.push_back("WRTau_ULID");                                                                                                                    


  for (auto i: ELIDs) {
    channel.push_back(EE);
    MuIDs.push_back(make_pair("POGHighPt","POGLoose")); //
  }
    
  MuIDs.push_back(make_pair("POGHighPtWithLooseTrkIso", "POGLoose"));
  paramnames.push_back("WRTau_ULID");
 
  int iel= MuIDs.size() - channel.size();
  for (int i =0; i < iel; i++){
    channel.push_back(MuMu);
    ELIDs.push_back(make_pair("CutBasedLooseNoIso","passVetoID"));
  }
...
void WRTau_FakeRate::RunM(std::vector<Electron> loose_el,  std::vector<Muon> loose_mu, std::vector<Jet> jets,   AnalyzerParameter param,  float event_weight){
...
void WRTau_FakeRate::RunE( std::vector<Electron> loose_el, std::vector<Muon> loose_mu, std::vector<Jet> jets, AnalyzerParameter param,  float event_weight){
...
double WRTau_FakeRate::ApplyNvtxReweight(int NPV, TString Key){
...
void WRTau_FakeRate::GetFakeRateAndPromptRates(AnalyzerParameter param, std::vector<Lepton *> leps,std::vector<bool> blepsT, std::vector<Jet>    jetCollTight, TString label, float event_weight, float isocut){
...
void WRTau_FakeRate::MakePromptRatePlots(TString label, TString tag,AnalyzerParameter param,  std::vector<Lepton *> leps , std::vector<bool> blepsT , std::vector<Jet> jets,  float event_weight, float isocut, Particle MET){
...
float WRTau_FakeRate::GetPrescale(std::vector<Lepton *>   leps  ){
...
void WRTau_FakeRate::MakeFakeRatePlots(TString label, TString mutag,AnalyzerParameter param,  std::vector<Lepton *> leps , std::vector<bool> blepsT ,  std::vector<Jet> jets,  float event_weight, float isocut, Particle MET){
...
bool WRTau_FakeRate::UseEvent(std::vector<Lepton *> leps ,  std::vector< Jet> jets, float awayjetcut, Particle MET, float wt){
...
void WRTau_FakeRate::GetFakeRates(std::vector<Lepton *> leps,std::vector<bool> blepsT,  AnalyzerParameter param,TString tightlabel,  std::vector<Jet> jets,  TString tag,float event_weight, float isocut){
...
void WRTau_FakeRate::FillRegionPlots( TString plot_dir, TString region,  std::vector<Jet> jets,  std::vector<Electron> els, std::vector<Muon> mus, Particle  met,  double w){
...
void WRTau_FakeRate::FillEventCutflow(int charge_i, int cf,float wt, TString cut,   TString label){
...
void WRTau_FakeRate::FillWeightHist(TString label, double _weight){
  FillHist( "weights/"+ label , _weight ,1., 200, 0., 5,"ev weight");
}
void WRTau_FakeRate::FillCutFlow(bool IsCentral, TString suffix, TString histname, double weight){
